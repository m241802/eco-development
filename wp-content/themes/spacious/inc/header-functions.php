					...
					if( !empty( $spacious_header_title ) || !empty( $spacious_slider_text ) || !empty( $spacious_slider_image ) ) {
						...
						<div class="<?php echo $classes; ?>">
							<figure>
								...
								<?php 
								     $attach_id = get_attachment_id_from_src(esc_url( $spacious_slider_image ));
								     $img_url_full = wp_get_attachment_image_src($attach_id, 'featured')/*[0]*/;
			                         $img_url = wp_get_attachment_image_src($attach_id, 'featured-blog-medium-small')/*[0]*/; 
			                        //print_r($img_url[0]);
			                         $min_img_slider[$i] = $img_url[0];

								//print_r(esc_url( $spacious_slider_image )); ?>
							</figure>
